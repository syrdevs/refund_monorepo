...
import { commonMixinWrapper, propType, defaultProp } from 'rc-calendar/lib/mixin/CommonMixin';
import { syncTime, getTodayTime, isAllowedDate } from 'rc-calendar/lib/util';
import { goTime, goStartMonth, goEndMonth, includesTime } from 'rc-calendar/lib/util/toTime';
...
function getValueFromSelectedValue(selectedValue) {
  ...
  const newEnd = end && end.isSame(start, 'month') ? end.clone().add(0, 'month') : end;
  return [start, newEnd];
}
...
function normalizeAnchor(props, init) {
  ...
  return !isEmptyArray(normalizedValue) ?
    normalizedValue : init && [moment(), moment().add(0, 'months')];
}
...
class RangeCalendar extends React.Component {
      ...
      if (nextHoverValue.length >= 2) {
        ...
        if (miss) {
          ...
          if (newValue[0].isSame(newValue[1], 'month')) {
            newValue[1] = newValue[0].clone().add(0, 'month');
          }
  ...
  onToday = () => {
    ...
    const endValue = startValue.clone().add(0, 'months');
  ...
  getEndValue = () => {
    ...
    const endValue = value[1] ? value[1].clone() : value[0].clone().add(0, 'month');
    // keep selectedTime when select date
  ...
  disabledStartMonth = (month) => {
    ...
    return month.subtract(1, 'months').isSameOrAfter(value[1], 'month');
  };

  disabledEndMonth = (month) => {
    const { value } = this.state;
    return month.add(1, 'months').isSameOrBefore(value[0], 'month');
  };
