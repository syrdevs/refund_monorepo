    ...
    APIClient.doApiCall = function (url, method, data, params, tokenRequired, contentType, responseType) {
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                ...
                headers = {};
                if (tokenRequired) {
                    authToken = utils_1.callAppFunction('getAuthToken');
                    if (!authToken) {
                        console.log('No auth token provided');
                        history_1.getHistory().push('/login');
                    }
                ...
                if (axios_1.default.interceptors.request['handlers'].length === 0) {
                    axios_1.default.interceptors.request.use(function (config) {
                        utils_1.requestStarted();
                        return config;
                    });
                }
                // tslint:disable-next-line:no-string-literal
                if (axios_1.default.interceptors.response['handlers'].length === 0) {
                    axios_1.default.interceptors.response.use(function (response) {
                        utils_1.requestFinished();
                        return response;
                    }, function (error) {
                        utils_1.requestFinished();
                        if (error.response && error.response.status === 401) {
                            antd_1.message.error(error.response.statusText);
                            history_1.getHistory().push('/login');
                        }
                        return Promise.reject(error);
                    });
                }
                return [2 /*return*/, axios_1.default(requestObj).catch(function (error) {
